...
def test_get_boxes(model, img,target, device,threshold=0.008, img_idx=1):
    # 预测框
    model.eval()
    # 得到真实框
    ...
    with torch.no_grad():
        ...
        scores, labels, boxes = model(img.unsqueeze(0))  #cpu版本
        #scores, labels, boxes = model(img.unsqueeze(0).cuda())  # cuda版本
        # 框的得分 ，标签，以及位置
